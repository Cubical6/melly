...
file_info:
  name: valet.md
  path: /home/user/melly/knowledge-base/libraries/laravel-12/valet.md
  size: 25088
structure:
  headings_count: 42
  code_blocks_count: 30
  internal_links_count: 23
processed_at: '2025-11-21T15:35:30.422123Z'
---
...
1. If you've added `.valetphprc` files to customize your site's PHP version, rename each `.valetphprc` file to `.valetrc`. Then, prepend `php=` to the existing content of the `.valetrc` file.
2. Update any custom drivers to match the namespace, extension, type-hints, and return type-hints of the new driver system. You may consult Valet's [SampleValetDriver](https://github.com/laravel/valet/blob/d7787c025e60abc24a5195dc7d4c5c6f2d984339/cli/stubs/SampleValetDriver.php) as an example.
3. If you use PHP 7.1 - 7.4 to serve your sites, make sure you still use Homebrew to install a version of PHP that's 8.0 or higher, as Valet will use this version, even if it's not your primary linked version, to run some of its scripts.
4. Determine if the driver serves the request.
5. Determine if the incoming request is for a static file.
6. @return string|false
7. Get the fully resolved path to the application's front controller.
