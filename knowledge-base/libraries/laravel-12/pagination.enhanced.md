...
file_info:
  name: pagination.md
  path: /home/user/melly/knowledge-base/libraries/laravel-12/pagination.md
  size: 22420
structure:
  headings_count: 21
  code_blocks_count: 23
  internal_links_count: 22
processed_at: '2025-11-21T15:35:23.222349Z'
---
...
1. Show all application users.
2. For large data-sets, cursor pagination will offer better performance if the "order by" columns are indexed. This is because the "offset" clause scans through all previously matched data.
3. For data-sets with frequent writes, offset pagination may skip records or show duplicates if results have been recently added to or deleted from the page a user is currently viewing.
4. Like `simplePaginate`, cursor pagination can only be used to display "Next" and "Previous" links and does not support generating links with page numbers.
5. It requires that the ordering is based on at least one unique column or a combination of columns that are unique. Columns with `null` values are not supported.
6. Query expressions in "order by" clauses are supported only if they are aliased and added to the "select" clause as well.
7. Query expressions with parameters are not supported.
